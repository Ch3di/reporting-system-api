...
struct CreateReportCategory: Migration {
    func prepare(on database: Database) -> EventLoopFuture<Void> {
        return database.schema("reports_categories")
                .id()
                .field("report_id", .uuid)
                .field("category_id", .uuid)
                .foreignKey("report_id", references: Report.schema, .id, onUpdate: .cascade)
                .foreignKey("category_id", references: Category.schema, .id, onUpdate: .cascade)
                // .unique(on: "report_id", "category_id")
                .create()
    }

    func revert(on database: Database) -> EventLoopFuture<Void> {
        return database.schema("reports_categories").delete()
    }
}
//
//import Foundation
